<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="text-xl font-semibold text-slate-900 leading-tight">Edit Category</h2>
            <p class="text-sm text-slate-500 mt-1">Update the category name.</p>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-xl mx-auto px-6">
            <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <form method="POST" action="{{ route('pharmacist.categories.update', $category) }}" class="space-y-5">
                    @csrf
                    @method('PUT')
