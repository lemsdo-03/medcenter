<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="text-xl font-semibold text-slate-900 leading-tight">Add Medicine</h2>
            <p class="text-sm text-slate-500 mt-1">Add a new medicine to the inventory.</p>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto px-6">
            <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <form method="POST" action="{{ route('pharmacist.medicines.store') }}" class="space-y-5">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <input id="min_quantity" name="min_quantity" type="number" min="0" value="{{ old('min_quantity', 10) }}" required
                                class="mt-1 block w-full rounded-2xl border-slate-200 bg-slate-50 focus:bg-white focus:border-emerald-300 focus:ring-0" />
